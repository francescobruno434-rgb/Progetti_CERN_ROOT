...
//funz_fondo_anti_csi->SetNormalized(1);
cout<<"l'integrale della polinomiale è "<< funz_fondo_anti_csi->Integral(1.3, 1.35)<<endl;
//
...
for(int i=0; i<len; i++){
    tree->GetEntry(i);
    if(charge>=0 ){//piglio solo le anti csi che hanno carica maggiore di 0
        int num_bin= hmass_anti_csi->GetBin(mass_csi); //questo è il numero del bin dell'istogramma delle masse iniziali a cui corrisponde il dato
        double bin_min= hmass_anti_csi->GetBinLowEdge(num_bin); //massa minore del bin
        double bin_width= hmass_anti_csi->GetBinWidth(num_bin); //larghezza del 
        double bin_max= bin_min+bin_width; //massa maggiore del bin 

        
        W= funz_segnale_anti_csi->Eval(mass_csi)/funz_anti_csi->Eval(mass_csi);//valore del peso
        
        a_W= 1-W; //valore dell'antipeso}
        //if(mass_csi>1.321 && mass_csi<1.3212) cout<< mass_csi<<" "<<W<<" "<<a_W<<endl;
